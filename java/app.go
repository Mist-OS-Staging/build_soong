...
func AutogeneratedRroModuleName(ctx android.EarlyModuleContext, moduleName, partition string) string {
	...
	product := strings.Replace(ctx.Config().Getenv("TARGET_PRODUCT"), "lineage_", "mist_", 1)
	overrideName := strings.Replace(strings.Replace(strings.Replace(moduleName, "lineageos", "mistos", -1), "Lineage", "Mist", -1), "lineage", "mist", -1)
